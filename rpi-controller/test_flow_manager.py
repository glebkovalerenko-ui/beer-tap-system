...
def test_flow_manager_shows_progress_and_clamps_volume():
    clock = FakeClock()
    buffer = io.StringIO()
    hardware = FakeHardware(
        card_present_responses=[True, True, True, True],
    ...
    assert "Pouring: 15 ml / 30 ml | est. cost: 30 cents" in output_lines
    assert "Pouring: 30 ml / 30 ml | est. cost: 60 cents" in output_lines
    assert output_lines[-1] == "Session finished. Poured: 30 ml"
    assert db_handler.pours[0]["volume_ml"] == 30
    assert db_handler.pours[0]["price_cents"] == 60
    ...
    sync_manager = FakeSyncManager(
        {
            ...
            "reason": "balance_below_min_start",
            "status_code": 403,
        }
    )
    created = {"count": 0}
