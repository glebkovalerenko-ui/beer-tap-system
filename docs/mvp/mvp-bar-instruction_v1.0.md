# [DEPRECATED/ARCHIVE] Техническая документация: Проект "Бар самообслуживания" (MVP v1.0)
...
| RFID Считыватель (Кран)    | USB NFC Reader (ACR122U or compatible PC/SC) | Идентификация гостя на точке налива            |
...
**Электромагнитный клапан 12V:**
- Подключается через реле к внешнему блоку питания 12V
- Нормально закрытый (Normal Closed)

**USB NFC Reader:**
- Подключается к USB порту Raspberry Pi
- Работает через PC/SC интерфейс (пакеты pcscd, libpcsclite-dev)
- Использует библиотеку pyscard для работы с картами MIFARE
...
#### **F.4 FlowManager - Конечный автомат управления наливом**

`FlowManager` реализует конечный автомат для управления процессом налива напитка:

**Состояния:**
1. **IDLE (Ожидание):** Начальное состояние, система ожидает RFID-карту
2. **AUTH (Авторизация):** Проверка карты на сервере
3. **POURING (Налив):** Активная сессия налива с открытым клапаном
4. **FINALIZING (Завершение):** Сохранение данных о наливе
5. **CARD_REMOVAL_WAIT (Ожидание удаления карты):** Ожидание удаления карты перед возвратом в IDLE

**Переходы:**
```
IDLE → AUTH: Карта обнаружена (is_card_present() = true)
AUTH → POURING: Карта авторизована (check_card_auth() = true)
AUTH → CARD_REMOVAL_WAIT: Карта не авторизована
POURING → FINALIZING: Карта удалена ИЛИ таймаут 15 сек
POURING → FINALIZING: Emergency stop активирован
FINALIZING → CARD_REMOVAL_WAIT: Данные о наливе сохранены (volume > 1мл)
CARD_REMOVAL_WAIT → IDLE: Карта удалена
```

**Логика состояния POURING:**
- Клапан открыт (`valve_open()`)
- Мониторинг потока каждые 0.5 сек
- Счетчик объема `total_volume_ml` накапливается при наличии потока
- Таймаут: 15 секунд без потока → автоматическое закрытие
- Emergency Stop: проверка каждые 3 секунды
- Генерация `client_tx_id` для трассировки транзакции

**Обработка ошибок:**
- Любые исключения в состоянии POURING приводят к `valve_close()` в `finally` блоке
- Минимальный объем для сохранения: 1 мл
- Все операции с объемом потокобезопасны через `threading.Lock`
