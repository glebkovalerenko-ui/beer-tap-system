...
## 8) Статус выполнения Этапа 2 («инвесторский экран ценности»)

Реализовано в коде:
1. Добавлен новый компонент `InvestorValuePanel.svelte` на дашборд с KPI-блоком:
   - выручка за сегодня,
   - объем продаж (литры),
   - средний чек,
   - активные/заблокированные краны,
   - топ-напиток дня.
2. Добавлен блок «Операционные риски» (critical/warning/info) для фокуса владельца бара на actionable-сигналах.
3. Дашборд теперь подтягивает данные кег (`kegStore.fetchKegs()`), чтобы риск-аналитика по остаткам была основана на реальных данных.

Что осталось для полного закрытия Этапа 2:
- добавить тренды по времени (не только snapshot за сегодня);
- расширить KPI маржинальностью (потребуются закупочные/себестоимостные данные в аналитическом срезе);
- добавить режим «демо-истории» с объяснением бизнес-выгоды на языке владельца (guided narrative).

## 9) Статус выполнения Этапа 3 («продуктовая зрелость»)

Реализовано в коде:
1. **Дизайн-система / UI-токены (первый срез)**
   - Добавлен файл `admin-app/src/styles/tokens.css` с базовыми design tokens (цвета, радиусы, тени, spacing).
   - Токены подключены глобально через `main.js`, и использованы в ключевых layout-компонентах.

2. **Guided demo mode**
   - Добавлен `DemoGuide.svelte` с пошаговым сценарием презентации (value -> operations -> guests).
   - Добавлен `demoGuideStore` для управления открытием/закрытием guided режима.

3. **Role-based интерфейс + аудит UI-действий**
   - Добавлен `roleStore` с ролями: владелец / менеджер / бармен и permission-моделью.
   - Реализована role-based навигация и role-aware поведение экранов (`Dashboard`, `Guests`).
   - Добавлен `auditTrailStore` и визуальный `ActivityTrail.svelte` (аудит событий UI: смена роли, запуск demo-guide).

Оставшийся scope Этапа 3 для полного enterprise-уровня:
- вынести больше компонентов на уровень UI-kit primitives (Button/Card/Badge/Input) с единым API;
- добавить персистентный аудит UI-действий на backend (не только in-memory/store);
- реализовать role policies на backend-side ACL (текущий role-based слой — UX-уровень).
