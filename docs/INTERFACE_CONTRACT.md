...
- If active lock exists but `pending_sync` is missing, backend returns `audit_only` with `outcome="audit_missing_pending"`, writes `sync_missing_pending`, and clears the stale lock.
