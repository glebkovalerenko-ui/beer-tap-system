...
| Безопасность backend | `backend/security.py` | JWT + internal token для контроллеров | ок (с замечаниями) | Ключи читаются из env (`SECRET_KEY`, `INTERNAL_API_KEY`, `INTERNAL_API_KEYS`, `INTERNAL_TOKEN`), но для dev есть fallback-значения. |
...
| CI | `.github/workflows/docs.yml`, `.github/workflows/quality.yml` | Docs + quality checks | ок (с замечаниями) | Добавлены backend/frontend проверки; рекомендуется расширить rust-check и link-check docs. |
...
| Используется dual-auth: JWT для админки + internal token для RPi | ✅ | `backend/security.py` (`oauth2_scheme`, `_get_internal_api_keys`, `get_current_user`) + `rpi-controller/sync_manager.py` (`X-Internal-Token`) | Поддерживать ротацию через `INTERNAL_API_KEYS` и убрать dev fallback в production. |
...
| CI проверяет качество кода и тесты | ✅ частично | Добавлен `quality.yml` с backend smoke (`pytest backend/tests/test_security.py`) и frontend build. | Добавить `cargo check` для tauri и docs link-check. |
...
| AUD-003 | безопасность/дока | S2 | В production недопустимы fallback-ключи по умолчанию | `backend/security.py`, `.env.example` | Ввести fail-fast при default `SECRET_KEY` вне dev и задокументировать hard requirement. | Med |
...
| AUD-005 | архитектура/дока | S3 | CI quality покрыт частично (нет rust/docs link checks) | `.github/workflows/quality.yml` | Добавить `cargo check` и docs link checker в CI. | Low |
...
1. Актуализировать `docs/INTERFACE_CONTRACT.md`: убрать ссылку на `src-tauri/src/lib.rs` как source of truth.
2. Добавить в CI `cargo check` для `admin-app/src-tauri`.
3. Добавить docs link-check для `README.md` + `docs/*.md`.
