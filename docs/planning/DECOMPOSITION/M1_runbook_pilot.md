...
```bash
docker-compose exec -e DATABASE_URL="postgresql://${POSTGRES_USER}:${POSTGRES_PASSWORD}@postgres:5432/${SCRATCH_DB}" beer_backend_api alembic stamp "$BASELINE_REV"
```

```bash
docker-compose exec -e DATABASE_URL="postgresql://${POSTGRES_USER}:${POSTGRES_PASSWORD}@postgres:5432/${SCRATCH_DB}" beer_backend_api alembic upgrade head
```

### 3.3 Validate revision state on scratch DB
```bash
docker-compose exec -e DATABASE_URL="postgresql://${POSTGRES_USER}:${POSTGRES_PASSWORD}@postgres:5432/${SCRATCH_DB}" beer_backend_api alembic current
```
