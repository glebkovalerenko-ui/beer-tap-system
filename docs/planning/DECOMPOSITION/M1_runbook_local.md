...
### 1.3 Apply migrations to head
```bash
docker-compose run --rm beer_backend_api alembic upgrade head
```

### 1.4 Verify Alembic revision state
```bash
docker-compose run --rm beer_backend_api alembic current
```

```bash
docker-compose run --rm beer_backend_api alembic history
```
...
### 2.2 Ensure backend container is running
```bash
docker-compose ps
```

### 2.3 Check service health endpoint
```bash
...
### 2.4 Optional docs endpoint check
```bash
