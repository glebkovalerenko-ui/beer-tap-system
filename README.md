...
[![Статус проекта: Demo-Ready](https://img.shields.io/badge/status-Demo--Ready-brightgreen)](https://shields.io/)
[![Технологии: Svelte 5, Tauri 2, FastAPI, PostgreSQL, SQLite](https://img.shields.io/badge/stack-Svelte%205%20|%20Tauri%202%20|%20FastAPI%20|%20PostgreSQL%20|%20SQLite-blue)](https://shields.io/)
...
Перед локальным запуском создайте `.env` на основе шаблона `.env.example` и задайте безопасные значения для `SECRET_KEY`, `INTERNAL_API_KEY` (и при необходимости `INTERNAL_TOKEN` для совместимости с RPi-контроллером).
...
6.  [🧠 Углубленный анализ: Взгляд экспертов](#🧠-углубленный-анализ-взгляд-экспертов)
    ...
    *   [Инженер встроенных систем (Embedded)](#5-инженер-встроенных-систем-embedded)
    *   [QA-инженер](#6-qa-инженер)
7.  [🗺️ Дорожная карта: от Demo-Ready к v1.0](#🗺️-дорожная-карта-от-demo-ready-к-v10)
...
├── admin-app/             # 🖥️ Лицо - Десктопное приложение (Svelte 5 + Tauri)
│   ├── src/              #    - Исходный код Svelte-приложения
│   ├── src-tauri/        #    - Rust бэкенд для доступа к NFC
│   │   └── src/main.rs   #    - Основной файл Rust с NFC-логикой
...
- **Многостадийные Docker-сборки:** `Dockerfile` для `admin-app` использует multi-stage build. На первом этапе (`build`) используется большой образ `node:22-alpine` для установки зависимостей и сборки статических файлов. На втором этапе используется крошечный образ `nginx:1.21-alpine`, куда копируются **только** результаты сборки. Это позволяет уменьшить размер финального образа с сотен мегабайт до ~20 МБ.
...
- **Архитектура: Tauri Backend (Rust) <-> Frontend (Svelte 5):** Гибридная архитектура, где Rust-бэкенд через Tauri обеспечивает прямой доступ к NFC-оборудованию, а Svelte 5 отвечает за пользовательский интерфейс. Svelte 5 обеспечивает отличную производительность и небольшой размер бандла, а Tauri 2.0 позволяет создавать нативные десктопные приложения с веб-технологиями.

- **Прямой доступ к NFC через Rust:** Rust-бэкенд отвечает за низкоуровневую работу с NFC-считывателем ACR122U через PC/SC интерфейс, обеспечивая надежную и быструю коммуникацию с оборудованием.

- **Полная русификация интерфейса:** Весь пользовательский интерфейс полностью переведен на русский язык, включая все кнопки, метки, сообщения об ошибках, заголовки и подсказки для удобства использования.
...
- **Маршрутизация:** Использование встроенной маршрутизации Svelte превращает приложение в SPA (Single Page Application), что обеспечивает быстрый и плавный переход между разделами без перезагрузки всей страницы.

### 5. Инженер встраиваемых систем (Embedded)
**Задача:** Написать надежное ПО для Raspberry Pi, работающее с физическим оборудованием.

- **Выбор gpiozero с LGPIOFactory:** Использует библиотеку gpiozero с LGPIOFactory для надежного управления GPIO пинами Raspberry Pi, обеспечивая стабильную работу с реле и датчиками потока.

- **Выбор PC/SC:** Это стандартный интерфейс для работы со смарт-картами, который обеспечивает надежную совместимость с RFID-считывателями.
...
## 🗺️ Дорожная карта: от Demo-Ready к v1.0
