...
import schemas
import security
from crud import card_crud, guest_crud, shift_crud, transaction_crud
...
@router.post("/{guest_id}/refund", response_model=schemas.Guest, summary="Оформить возврат гостю")
def refund_guest_balance(
    guest_id: uuid.UUID,
    ...
    shift_crud.ensure_open_shift(db)
    return transaction_crud.create_refund_transaction(db=db, guest_id=guest_id, refund_data=refund_data)
...
def get_guest_history(
    ...
    return guest_crud.get_guest_history(db=db, guest_id=guest_id, start_date=start_date, end_date=end_date)
