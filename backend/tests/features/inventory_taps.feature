...
Функционал: Управление Инвентарем и Кранами
  ...
  @TC-API-INV-04
  Сценарий: Попытка создать кегу с несуществующим напитком
    Дано Администратор авторизован
    Когда Клиент отправляет POST-запрос на /api/kegs/ с несуществующим beverage_id
  ...
  @TC-API-INV-06
  Сценарий: Успешное удаление неиспользуемой кеги
    Дано Администратор авторизован
    И Существует кега со статусом "full"
    Когда Клиент отправляет DELETE-запрос на /api/kegs/{keg_id}
    Тогда API должен вернуть ответ с кодом 204
  ...
  @TC-API-INV-08
  Сценарий: Попытка удалить кран с подключенной кегой
    Дано Администратор авторизован
    И К крану {tap_id} привязана кега
    Когда Клиент отправляет DELETE-запрос на /api/taps/{tap_id}
    Тогда API должен вернуть ответ с кодом 400
