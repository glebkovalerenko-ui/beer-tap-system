...
Функционал: Управление Гостями и Финансами
  ...
  @TC-API-GST-04
  Сценарий: Попытка создания гостя без обязательного поля
    Дано Администратор авторизован
    Когда Клиент отправляет POST-запрос на /api/guests/ без поля "last_name"
...
@TC-API-GST-07
  Сценарий: Попытка привязать уже занятую карту
    Дано Администратор авторизован
    И Карта "TAKEN-UID-456" привязана к другому гостю
    Когда Клиент пытается привязать эту карту к новому гостю
    Тогда API должен вернуть ответ с кодом 409
  ...
  Сценарий: Успешное отвязывание карты от гостя
    ...
    Тогда API должен вернуть ответ с кодом 200
    И Статус карты "ACTIVE-UID-789" в БД должен стать "available"
  ...
  @TC-API-GST-11
  Сценарий: Попытка пополнить баланс несуществующему гостю
    Дано Администратор авторизован
    Когда Клиент отправляет POST-запрос на /api/guests/{несуществующий_uuid}/topup
    Тогда API должен вернуть ответ с кодом 404

  @TC-API-GST-12
  Сценарий: Успешное получение смешанной истории операций
    Дано Администратор авторизован
    И У гостя есть пополнение и налив
    Когда Клиент отправляет GET-запрос на /api/guests/{guest_id}/history
    Тогда API должен вернуть ответ с кодом 200
    И Тело ответа должно содержать отсортированный список из двух операций
