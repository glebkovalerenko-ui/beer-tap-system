...
@skip @TC-API-CTL-05
  ...
  Сценарий: Отклонение налива из-за неактивного крана
    Дано Кран имеет статус "locked"
    Когда Контроллер отправляет налив с этого крана
    Тогда API должен вернуть 200 OK со статусом "rejected"

@skip  @TC-API-CTL-09
  Сценарий: Налив, опустошающий кегу
    Дано Остаток в кеге равен объему налива
    Когда Контроллер отправляет налив, который должен опустошить кегу
    Тогда API должен вернуть 200 OK со статусом "accepted"
    И Статус кеги и крана должен измениться на "empty"